...
t<-read.csv(paste(path,"Data/Summaries/Groundfish_Sets.csv",sep=""),stringsAsFactors = F)
...
for (num_col in names(t)) {
  ...
  pdf(paste(path, "Results/Figures/Covariates/Groundfish_commercial/", n, ".pdf", sep=""))
  par(mfrow = c(2,3))
  ...
  for (i in num_col) {
    ...
    plot(t$lon_dd, t$lat_dd, pch = 1, cex = (fish/1000), col = t$year2)
...
for (num_col in names(t)) {
  ...
  for (i in num_col) {
    ...
    plot(t$lon_dd, t$lat_dd, pch = 1, cex = 0.3, col = t$year2)
    #with(subset(t, fish == 0), points(lon_dd, lat_dd, pch = "x", col = "red", cex = 0.3, lwd = 0.8))
    legend("bottomright", legend=unique(t$year2), col=unique(t$year2), pch=1, cex=0.8)
...
# 
# path <- "/Users/user/DFO_2017/"
t<-read.csv(paste(path,"Data/Summaries/Temporal_Groundfish_Sets.csv",sep=""),stringsAsFactors = F)

pdf(paste(path, "Results/Figures/Covariates/Groundfish_commercial/temporal.pdf", sep=""))
par(mfrow = c(3,3))
cols = names(t[,c(2:56)])
for (i in cols) {
...
for (num_col in names(t)) {
  ...
  pdf(paste(path, "Results/Figures/Covariates/Groundfish_commercial/env", n, ".pdf", sep=""))
  par(mfrow = c(3,3))
  cols = names(f[,c(12:66)])
...
# For these ones I am only making the CDF, the data files are all but unusable otherwise due to the time it takes to render each point....
for (num_col in names(t)) {
  if(!(grepl("_kg", num_col))) {
    next
  }
  ...
  pdf(paste(path, "Results/Figures/Covariates/Groundfish_commercial/env_CDF_", n, ".pdf", sep=""))
  par(mfrow = c(3,3))
  cols = names(f[,c(12:66)])
  ...
  for (i in cols) {
    ...
    #f.cdf <- ecdf(cdf$fish)
    plot(cdf$cum_fish~cdf$env, xlab = e, ylab = n,type="l")
    ...
    #plot(fish ~ env, xlab = e, ylab = n,pch=19,cex=0.1)
    #abline(x)
