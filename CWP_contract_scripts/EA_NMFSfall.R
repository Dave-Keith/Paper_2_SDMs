...
t<-read.csv(paste(path,"Data/Summaries/NMFSfall.csv",sep=""),stringsAsFactors = F)
...
for (num_col in names(t)) {
  ...
  pdf(paste(path, "Results/Figures/Covariates/NMFSfall/", n, ".pdf", sep=""))
  par(mfrow = c(2,3))
...
#   fish <- t[,num_col]
#   
#   pdf(paste(path, "Results/Figures/NMFSfall/", n, ".pdf", sep=""))
#   par(mfrow = c(2,3))
#   
#   for (i in num_col) {
#     e <- i
...
#path <- "/Users/user/DFO_2017/"
 t<-read.csv(paste(path,"Data/Summaries/Temporal_NMFSfall.csv",sep=""),stringsAsFactors = F)

pdf(paste(path, "Results/Figures/Covariates/NMFSfall/temporal.pdf", sep=""))
par(mfrow = c(3,3))
cols = names(t[,c(2:55)])
for (i in cols) {
...
for (num_col in names(t)) {
  ...
  pdf(paste(path, "Results/Figures/Covariates/NMFSfall/env", n, ".pdf", sep=""))
  par(mfrow = c(3,3))
  cols = names(f[,c(20:74)])
...
for (num_col in names(t)) {
  ...
  pdf(paste(path, "Results/Figures/Covariates/NMFSfall/env_CDF_", n, ".pdf", sep=""))
  par(mfrow = c(3,3))
  cols = names(f[,c(20:74)])
