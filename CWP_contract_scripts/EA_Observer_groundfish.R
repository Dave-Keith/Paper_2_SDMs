...
##Use these datasets:
#Observers_Groundfish_Set.csv
#Temporal_Observers_Groundfish_Set.csv
...
t<-read.csv(paste(path,"Data/Summaries/Observers_Groundfish_Set.csv",sep=""),stringsAsFactors = F)
...
for (num_col in names(t)) {
  ...
  pdf(paste(path, "Results/Figures/Covariates/Observer_Groundfish/", n, ".pdf", sep=""))
  par(mfrow = c(2,3))
  ...
  for (i in num_col) {
    ...
    #with(subset(t, fish == 0), points(lon.dd, lat.dd, pch = "x", col = "red", cex = 0.3, lwd = 0.8))
...
# 
# path <- "/Users/user/DFO_2017/"
t<-read.csv(paste(path,"Data/Summaries/Temporal_Observers_Groundfish_Set.csv",sep=""),stringsAsFactors = F)

pdf(paste(path, "Results/Figures/Covariates/Observer_Groundfish/temporal.pdf", sep=""))
par(mfrow = c(3,3))
cols = names(t[,c(2:56)])
for (i in cols) {
...
for (num_col in names(t)) {
  ...
  pdf(paste(path, "Results/Figures/Covariates/Observer_Groundfish/env", n, ".pdf", sep=""))
  par(mfrow = c(3,3))
...
for (num_col in names(t)) {
  ...
  pdf(paste(path, "Results/Figures/Covariates/Observer_Groundfish/env_CDF_", n, ".pdf", sep=""))
  par(mfrow = c(3,3))
  cols = names(f[,c(25:66)])
