...
sliderInput("prob_pred_2", label = "Encounter Probability",
            min = 0, max = 1, value = 0.8, step = 0.05)
...
renderPlot({
  ...
  isolate({
    ...
    plt.pred <- bp.pred + geom_sf(data = preds() ,aes(fill = pred,colour=pred))+ sf + sc +
      geom_sf_text(data = area.era.pred() , aes(label = lab),parse=T, size=8)  + theme_map() + 
      theme(legend.key.height =hgt,text = element_text(size=22),legend.position = "right") +
      ggtitle(paste("Encounter probability \u2265",input$prob_pred_2, spec,surv))+
