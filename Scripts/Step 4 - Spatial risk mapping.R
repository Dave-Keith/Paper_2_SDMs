# Here's what I hope is the script that contains the final spatial INLA models used for the analysis reside, here's the idea...
...
# 2:  We include the top 4 PCA scores and a number of the environmental data as covariates, due to sanity limitations we only include 1 PCA score and see if any of these 
...
# 5:  In "Step 5" we will look at the subset of interesting models and develop spatio-temporal models for the subset of interesting models, that happens
#     in the script "Step 5 - Spatial_temporal_risk mapping.R"
...
#save.image(paste0(direct.proj,"Results/INLA_spatial_output.RData"))
#save.image(paste0(direct.proj,"Results/INLA_spatial_output_scal_2016.RData"))
#load(paste0(direct.proj,"Results/INLA_spatial_output.RData"))
  ...
  for(i in 1:num.surveys) 
  {
    ...
    if((surveys[i] == 'scallop' && !grepl("PA",species[s])) || (surveys[i] != 'scallop' && grepl("PA",species[s])))
    {
      ...
      for(m in 1:n.mods)
      {
        ...
        w.ind <- w.index.list[[paste("Base",species[s],surveys[i],sep="_")]]
      ...
      # This will be useful for when I do a final model selection, but not necessary yet I don't think
      # 
      # # Now make some residual plots, b/c it's binomial there aren't a lot of great ones, but residuals v.s. covariates can't hurt...
      # pdf(file = paste0(direct.proj,"/Results/Figures/INLA/Diagnostics/Residuals/Resid_vs_select_covariates_spatial",species[s],"_",surveys[i],".pdf"),
      ...
      pdf(paste0(direct.proj,"Results/Figures/INLA/GOM_and_GB_spatial_fields_",species[s],"_",surveys[i],".pdf"), width = 22, height = 11,onefile = T)
      # plot 2 per page
      ...
      for(m in 1:n.mods)
      {
        ...
        # If it is not a spatio-temporal model get this w index
        w.ind <- w.index.list[[paste("Base",species[s],surveys[i],sep="_")]]
        eras <- 1 # This is really for the spatio-temporal models, just easier to retain it than debug all the code below (which was originally designed
        # to work with spatial and spatio-temporal output.)
        ...
        # Except for the one categorical variable of SEDNUM, because of that dude, the figures from models with SEDNUM are all junk...
        # I think I could fix this easily by just adding ...$summary.fixed$mean[1] and that would just grab the first level and
        # would at least look decent :-)
        tmp.field <- data.frame(r.field.link = rand.field[[run.name]]$mean[w.ind$w.repl == eras] + res[[run.name]]$summary.fixed$mean, 
                                r.field.response = inv.logit(rand.field[[run.name]]$mean +  res[[run.name]]$summary.fixed$mean)[w.ind$w.repl == eras],
                                r.field.sd = rand.field[[run.name]]$sd[w.ind$w.repl == eras],
                                era = eras)
        # Get the points used for each of the models.
        loc <- data.frame( X = dat.list[[paste(species[s],surveys[i],sep="_")]]$X, Y = dat.list[[paste(species[s],surveys[i],sep="_")]]$Y)
        ...
        title(paste("Encounter Probability - All years",mod.names[m]))
...
com.ly <- NULL
com.ly <- list(res = res[["com_stan_ly scallop survey model.intercept"]],
                rand.field = rand.field[["com_stan_ly scallop survey model.intercept"]],
                mod.output = mod.output[["com_stan_ly scallop survey model.intercept"]],
                mod.diagnostics = mod.diagnostics[["com_stan_ly scallop survey model.intercept"]],
                mesh = mesh.sc)
rec.ly <- NULL
rec.ly <- list(res = res[["rec_stan_ly scallop survey model.intercept"]],
               rand.field = rand.field[["rec_stan_ly scallop survey model.intercept"]],
               mod.output = mod.output[["rec_stan_ly scallop survey model.intercept"]],
               mod.diagnostics = mod.diagnostics[["rec_stan_ly scallop survey model.intercept"]],
               mesh = mesh.sc)

save(rec.ly,com.ly,file = paste0(direct.proj,"Results/Scal_ly_results.RData"))
