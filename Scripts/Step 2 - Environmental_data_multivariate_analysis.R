...
# To do the variance inflation factor method you need the highland stats package, because I'm doing this in Edmonton I'm
# just going to put it in the SPERA directory, do better when back in Hali...
source(paste0(direct,"scripts/HighstatLibV11.R"))
...
variables <- c("botstr_wt",
               ...
               'sst_avg',
               'sst_rg',
               #'SEDNUM', Categroical vairable so remove from VIF and PCA
               'FID_GMAINE_median',
               'Year_median',
               'strat96',
               'sil_avg96',
               'chl_rg')
               #'growth' # I'm removing grwoth b/c it is mostly junk data...
...
######################################  VARIANCE INFLATION FACTOR METHOD ################################# VARIANCE INFLATION FAcTOR ######3
# As an alternative to doing the crazy PCA analyses which may or may not make sense, we can also simply pair down
# our environmental covariates using variance inflation factors and see what the best subset of data actually is
# THe rule of thumb is you can toss out anything with a VIF greater than 3, but do it one at a time, we should be
# able to automate this process.  I am only going to do this for the NMFS and RV surveys.


remove.nmfs.spr <- data.frame(var.name = rep(NA,length(variables)), VIF = rep(NA,length(variables)))
nmfs.spr.vif <- nmfs.spr.enviro[,!names(nmfs.spr.enviro) %in% c("sst_avg","comldepth")]
for(i in 1:ncol(nmfs.spr.vif))
{
  tmp <- corvif(nmfs.spr.vif)
  max.vif <- max(tmp$GVIF)
  if(max.vif < 3) stop("You've removed all variables with a VIF > 3, you are super!")
  rm <- which(tmp$GVIF == max.vif)
  remove.nmfs.spr$var.name[i] <- row.names(tmp)[rm][1]
  remove.nmfs.spr$var.name[i] <- max.vif
  # Now actually remove that from the data.
  nmfs.spr.vif <- nmfs.spr.vif[,-which(names(nmfs.spr.vif) == row.names(tmp)[rm])]
}
# Now repeat for nmfs.fall
remove.nmfs.fall <- data.frame(var.name = rep(NA,length(variables)), VIF = rep(NA,length(variables)))
nmfs.fall.vif <- nmfs.fall.enviro[,!names(nmfs.fall.enviro) %in% c("sst_avg","comldepth")]
for(i in 1:ncol(nmfs.fall.vif))
{
  tmp <- corvif(nmfs.fall.vif)
  max.vif <- max(tmp$GVIF)
  if(max.vif < 3) stop("You've removed all variables with a VIF > 3, you are super!")
  rm <- which(tmp$GVIF == max.vif)
  remove.nmfs.fall$var.name[i] <- row.names(tmp)[rm][1]
  remove.nmfs.fall$var.name[i] <- max.vif
  # Now actually remove that from the data.
  nmfs.fall.vif <- nmfs.fall.vif[,-which(names(nmfs.fall.vif) == row.names(tmp)[rm])]
}

# And finally the RV survey data....
remove.RV <- data.frame(var.name = rep(NA,length(variables)), VIF = rep(NA,length(variables)))
rv.vif <- RV.surv.enviro[,!names(RV.surv.enviro) %in% c("sst_avg","comldepth")]
for(i in 1:ncol(rv.vif))
{
  tmp <- corvif(rv.vif)
  max.vif <- max(tmp$GVIF)
  if(max.vif < 3) stop("You've removed all variables with a VIF > 3, you are super!")
  rm <- which(tmp$GVIF == max.vif)
  remove.RV$var.name[i] <- row.names(tmp)[rm][1]
  remove.RV$var.name[i] <- max.vif
  # Now actually remove that from the data.
  rv.vif <- rv.vif[,-which(names(rv.vif) == row.names(tmp)[rm])]
}

# Now which of these are common to all 3 analyses? The ones in the RV are common to all, I've also kept SST average and depth because
# those two are of interest to many...
names(rv.vif)
names(nmfs.fall.vif)
names(nmfs.spr.vif)
# Keep   "botstr_wt"         "complexity"        "comlaspect"        "Mud"               "sal_avg96"         "sal_rg96"          "phos_avg96"       
#        "comlslope"         "sand"              "FID_GMAINE_median" "strat96"           "sil_avg96"         "chl_rg"  
# I also remove growth because that data is mostly garbage...
# Also include depth since we know how important it is.  I'm not going to run these
# for every model out there, think about what models we want to run these for, I will
# do this with forward model selection and see if any of these terms improves the models over the NULL model
# I will still do the PCA analysis and see if anything interesting comes up consistently.
# I will also do the model selection on only the spatial models, not the spatial-temporal as the first step
# then with the chosen models I'll run them with the spatio-temporal fields and see what they look like.
# This should save decades of computing time!

vif.variables <- c("botstr_wt" ,"complexity", "comlaspect", "Mud",  "sal_avg96" ,"sal_rg96", "phos_avg96", "comlslope",
                   "sand", "sst_avg", "SEDNUM","strat96","comldepth","FID_GMAINE_median","sil_avg96","chl_rg")
...
# So make another subset that is the vif variables only, we'll mergfe these after we make the PCA scores below to have a huge complete dataset
nmfs.spr.vif <- nmfs.spr[,c(non.enviro.surv.dat,vif.variables)]
nmfs.fall.vif <- nmfs.fall[,c(non.enviro.surv.dat,vif.variables)]
RV.surv.vif <- RV.surv[,c(non.enviro.surv.dat,vif.variables)]
...
nmfs.spr.final <- nmfs.spr.4pca[,c("unique_set_ID",names(nmfs.spr.4pca)[grep("PC",names(nmfs.spr.4pca))])]

nmfs.fall.final <- nmfs.fall.4pca[,c("unique_set_ID",names(nmfs.fall.4pca)[grep("PC",names(nmfs.fall.4pca))])]

RV.surv.final <- RV.surv.4pca[,c("unique_set_ID",names(RV.surv.4pca)[grep("PC",names(RV.surv.4pca))])]


# Now for the 3 surveys I want to add in the VIF variables we have.
nmfs.spr.final <- left_join(nmfs.spr[,c("unique_set_ID","lat_dd","lon_dd","year","COD_number_Adu","COD_PA_Adu",
                                         "YT_number_Adu","YT_PA_Adu",vif.variables,"strata")],nmfs.spr.final,by ="unique_set_ID")
nmfs.fall.final <- left_join(nmfs.fall[,c("unique_set_ID","lat_dd","lon_dd","year","COD_number_Adu","COD_PA_Adu",
                                        "YT_number_Adu","YT_PA_Adu",vif.variables,"strata")],nmfs.fall.final,by ="unique_set_ID")
RV.surv.final <- left_join(RV.surv[,c("unique_set_ID","lat_dd","lon_dd","year","COD_number_Adu","COD_PA_Adu",
                                        "YT_number_Adu","YT_PA_Adu",vif.variables,"strata")],RV.surv.final,by ="unique_set_ID")


names(RV.surv.final) <- c("unique_set_ID","lat","lon",'year','cod',"cod_PA",'yt','yt_PA',vif.variables,'strata',names(RV.surv.final)[grep("PC",names(RV.surv.final))])
...
##save.image(paste0(direct,"Results/PCA_and_vif_enviro_data.RData"))
