...
# The random fields
if(direct.proj != 'github')load(paste0(direct.proj,"Paper_2_SDMs/data/random_fields_from_top_models.RData"))
if(direct.proj == 'github')load(url("https://raw.githubusercontent.com/Dave-Keith/Paper_2_SDMs/master/data/random_fields_from_top_models.RData"))

# The meshes
if(direct.proj != 'github') load(paste0(direct.proj,"Paper_2_SDMs/data/INLA_meshes.RData"))
...
if(direct.proj != 'github') load(paste0(direct.proj,"Paper_2_SDMs/data/INLA_mesh_input_data.RData"))
...
if(direct.proj != 'github') load(paste0(direct.proj,"Paper_2_SDMs/data/Prediction_fields_all_models.RData"))
if(direct.proj == 'github') load(url("https://raw.githubusercontent.com/Dave-Keith/Paper_2_SDMs/master/data/Prediction_fields_all_models.RData"))
...
if(direct.proj == 'github') load(url("https://raw.githubusercontent.com/Dave-Keith/Paper_2_SDMs/master/data/INLA_2017_2019_prediction_error_summary.RData"))

# THe model diagnostics when comparing the 3/5/10 year fields...
if(direct.proj != 'github') load(paste0(direct.proj,"Paper_2_SDMs/data/INLA_st_3_5_10_model_diagnostics.RData"))
...
**The left column contains the results comparing the 3 random fields (Intercept only models), the right column compares different covariate models with using the best random field model. *Atlantic Cod* model results shown are from the *Winter* survey, *Yellowtail Flounder* model results are from the *Spring* survey.**
  ...
  {  
    ...
    field.plt <-ggplot(diag.fields) + geom_point(aes( x =response,y=as.factor(era)),size=2) + ylab("") + xlab(ax.lab) + 
     ...
     fixed.plt <- ggplot(diag.fixed) + geom_point(aes(x =response,y=model),size=2) + xlab(ax.lab)+ ylab("") + 
                                       geom_vline(aes(xintercept = min_2), col = 'blue',size=1.25) +
                                       geom_vline(aes(xintercept = min_10), col = 'red',size=1.25) + 
...
**These results show the time series of core area during spawning on GB in the U.S. and within Canada using the Canadian Offshore Scallop Fishery (COSF) survey domain. *Atlantic Cod* model results shown are from the *Winter* survey, *Yellowtail Flounder* model results are from the *Spring* survey, both stocks use the 5-year random field model for prediction.**
...
#input <- list(species_pred = 'cod',year_pred = 1996:2000,prob_pred = 0.5)

area.era.all <- reactive({
  # Make this run if we hit our go button, see how we added an isolate command to all the places in which we were using an input$ object to stop recalcs.
  input$go_pred
  isolate({
    if(input$species_pred == 'cod') preds.all <- pred.dat[[paste0(input$species_pred,"_PA RV_survey",sep="")]]
    if(input$species_pred == 'yt') preds.all <- pred.dat[[paste0(input$species_pred,"_PA nmfs-spring_survey",sep="")]]
    ...
    for(n in min(eras):max(eras))
    {
      yrs <- paste0(substr(dat.final %>% filter(years_5 == n, survey == unique(preds.all$survey)) %>% summarise(min = min(year)),3,4),"-",
                    substr(dat.final %>% filter(years_5 == n, survey == unique(preds.all$survey)) %>% summarise(max = max(year)),3,4))
      if(substr(yrs[1],1,2) > 30) { yrs <- paste0(19,yrs)} else {yrs <- paste0(20,yrs)}
      preds.all$yrs[preds.all$years_5==n] <- yrs
      preds.all$first_year[preds.all$years_5==n] <- as.numeric(dat.final %>% filter(years_5 == n, survey == unique(preds.all$survey)) %>% summarise(min = min(year)))
    }
        # So the key is the last thing is the dataframe we want...
    st_geometry(preds.all) <- rep(st_geometry(mesh.grid),n.eras)
    ...
    if(input$closure == 'none') 
    {
      ...
      cls.area <- st_intersection(preds.all,gb.surv)
...
renderPlot({
  ...
  isolate({
     if(input$closure == 'none') area.title <- "Total core area on GB-US and in COSF"
...
renderImage(deleteFile=F,{
     ...
     isolate({
              if(input$closure == "all")
              {
                tmp <- tempfile()
                # Download this to the temp directory you created above
                download.file("https://raw.githubusercontent.com/Dave-Keith/Paper_2_SDMs/master/Dashboard/kitten.png", tmp)
                list(src = paste0(tmp, "\\kitten.png"),
                contentType = 'image/png')
              } else
                {
                tmp <- tempfile()
                # Download this to the temp directory you created above
                download.file("https://raw.githubusercontent.com/Dave-Keith/Paper_2_SDMs/master/Dashboard/blank.png", tmp)
                list(src = paste0(tmp, "/blank.png"),
                contentType = 'image/png')
                } # end else
     })
})
...
sliderInput("prob_pred_2", label = "Occurrence Probability",
            min = 0, max = 1, value = 0.75, step = 0.05)
radioButtons("species_pred_2","Which Species",
             choices = c("Atlantic cod" = "cod",
                         "Yellowtail flounder" = "yt"))
...
sliderInput('year_pred', "Survey Observations",min = 1972,max=2019,value = 1987:2016,step=1,dragRange = T)
...
preds.all <- reactive({
  ...
  input$go_pred_2
  isolate({
    if(input$species_pred_2 == 'cod') preds.all <- pred.dat[[paste0(input$species_pred_2,"_PA RV_survey",sep="")]]
    if(input$species_pred_2 == 'yt') preds.all <- pred.dat[[paste0(input$species_pred_2,"_PA nmfs-spring_survey",sep="")]]
    preds.all <- preds.all[,-which(names(preds.all) =='year')]
    n.eras <- length(unique(preds.all$years_5))
    eras <- factor.2.number(unique(preds.all$years_5))
    # So the key is the last thing is the dataframe we want...
    for(n in min(eras):max(eras))
    {
      ...
      preds.all$first_year[preds.all$years_5==n] <- as.numeric(dat.final %>% filter(years_5 == n, survey == unique(preds.all$survey)) %>% summarise(min = min(year)))
        ...
        # So the key is the last thing is the dataframe we want...
    st_geometry(preds.all) <- rep(st_geometry(mesh.grid),n.eras)
    preds.all <- preds.all %>% filter(pred >= input$prob_pred_2)
    preds.all$area <- preds.all %>% st_area() %>% set_units("km^2")
    preds.all
  })# end isolate
})
...
preds <- reactive({
  ...
  isolate({
    ...
    first.years <- unique(preds$first_year)
...
renderPlot({
  ...
  input$go_pred_2
  isolate({
    if(input$species_pred_2 == 'yt') {spec <- "Yellowtail"; surv <- "NMFS spring"}
    if(input$species_pred_2 == 'cod') {spec <- "Cod"; surv <- "RV survey"}
    col <- addalpha(pals::viridis(101),1)
    ...
    sf <- scale_fill_gradientn(colours = col, limits=lims,breaks=brk,name="Probability")
    sc <- scale_colour_gradientn(colours = col, limits=lims,breaks=brk,name="Probability")
    predict.pts <- all.resids %>% filter(year %in% min(input$year_pred):max(input$year_pred), species == paste0(input$species_pred_2,"_PA"))
