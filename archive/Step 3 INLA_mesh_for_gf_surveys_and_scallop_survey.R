...
{
  tmp <- as.vector(t(maxs[i,-1]))
  tmp2 <- t(scal.surv.dat[scal.surv.dat$year == years[i],names(scal.surv.dat) %in% names(maxs[-1])])
  res <- t(tmp2  / tmp)
...
dat.final$year.group <- 1
eras <- unique(floor(dat.final$year/10)*10)
n.eras <- length(eras)
...
dat.final$year.group <- as.numeric(as.factor(floor(dat.final$year/10)*10))
# Now do the same for the scallop, go with decades for now, since these data start in 1980 and the gf starts in 1970 I want
# to add 1 so that 1 represents 1970s, 2 is 1980's etc.
scal.surv.dat$year.group <- as.numeric(as.factor(floor(as.numeric(scal.surv.dat$year)/10)*10))+1
# Rename the lat/lon fields to lat and lon
names(dat.final)[grepl("dd",names(dat.final))] <- c("lat","lon")
...
save(dat.final,scal.surv.dat,yt.closed.cells,cod.closed.cells,file = paste0(direct.proj,"Data/INLA_mesh_input_data.RData"))
...
#  list(Polygon(bound2.gf$loc, FALSE)), '0')), proj4=prj4s)
...
plot(bound.gf.sp,xlim=c(350000,840000))
#plot(bound2.gf.sp, add=TRUE)
points(loc.gf, pch=19, col=grey(0.3,alpha=0.2), cex=0.2)
