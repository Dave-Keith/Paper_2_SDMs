...
df <- read.csv('d:/r/SPERA/Data/RV_survey_CA&US/DFO_GB_SPRING_tows_cod_ytf.csv')
#if rescaling - to centre - do so here
#df$y = df$r

# Load the mesh
load('d:/r/SPERA/Data/RV_survey_CA&US/INLA_mesh_RVsurvey_Georges.RData')


## 2. Explore data
summary(df)

hist(df$Cod_TOTNO,breaks=1000,xlim=c(0,10))
length(df$Cod_TOTNO[df$Cod_TOTNO>3.5])
length(df$Cod_TOTNO[df$Cod_TOTNO==0])
df$bino_cod <- df$Cod_TOTNO
df$bino_cod[df$bino_cod > 0] <- 1
df$bino_yt <- df$Ytf_TOTNO
df$bino_yt[df$bino_yt > 0] <- 1
...
res1 <- inla(formula1, family=family1, control.family=control.family1,data=df)
summary(res1)
# Mean count of cod
cod.mean.count <- exp(res1$summary.fixed[1])

# Now a binomial model...
res2 <- inla(formula2, family=family2, control.family=control.family2,data=df)
summary(res2)
...
stk.cod = inla.stack(tag="est",
                 data=list(y = df$bino_cod, link=1L),
                 effects=list(a0 = rep(1, nrow(df)), s = 1:spde$n.spde),
                 A = list(1, A))

stk.yt = inla.stack(tag="est",
                 data=list(y = df$bino_yt, link=1L),
                 effects=list(a0 = rep(1, nrow(df)), s = 1:spde$n.spde),
                 A = list(1, A))
...
load(paste("d:/r/SPERA/CWP/Output/Closure_biomass_VMS.RData",sep=""))
cells <- centres
...
local.plot.field(rands.cod, mesh,c(0,1),xlim = range(df$SLO),ylim = range(df$SLA))
local.plot.field(rands.yt, mesh,c(0,1),xlim = range(df$SLO),ylim = range(df$SLA))
