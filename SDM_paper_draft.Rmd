...
# Coorindates for CA1, CA2, and the cod/yellowtail closures
CA1 <- st_read(dsn ="d:/github/Offshore/GIS_layers/DK/other_boundaries/CA1.shp")
CA1 <- st_transform(CA1,crs = 32619)
CA2 <- st_read(dsn = "d:/github/Offshore/GIS_layers/DK/other_boundaries/CA2.shp")
CA2 <- st_transform(CA2,crs = 32619)
yt.closures <- st_read(dsn ="d:/github/Offshore/GIS_layers/DK/other_boundaries/yt_closures.shp")
cod.closures <- st_read(dsn = "d:/github/Offshore/GIS_layers/DK/other_boundaries/cod_closures.shp")
gba.surv <- st_read(dsn = "d:/github/Offshore/GIS_layers/DK/survey_boundaries/GBa.shp")
gbb.surv <- st_read(dsn = "d:/github/Offshore/GIS_layers/DK/survey_boundaries/GBb.shp")
...
# Then we can show how the random fields evolve
...
pred.fun <- function(species = 'cod',survey = "RV",prob = 0.75,mesh = mesh.grid,closure.yrs = 2007:2016)
{
...
plt.hi.yt.spring.bank <- ggplot(hi.yt.spring$bank) + geom_point(aes(y = as.numeric(tot.area),x = yrs)) + xlab("Era") + ylab(expression(paste("Area (",km^2,")")))
plt.hi.yt.spring.scal <- ggplot(hi.yt.spring$scal) + geom_point(aes(y = as.numeric(tot.area),x = yrs)) + xlab("Era") + ylab(expression(paste("Area (",km^2,")")))
plt.hi.yt.spring.scal.prop.GB <- ggplot(hi.yt.spring$scal) + geom_point(aes(y = as.numeric(prop.of.GB),x = yrs)) + xlab("Era") + ylab("Porportion")
plt.hi.yt.spring.scal.prop.can <- ggplot(hi.yt.spring$scal) + geom_point(aes(y = as.numeric(prop.scal.hi),x = yrs)) + xlab("Era") + ylab("Porportion")
...
plt.hi.yt.closure <- ggplot(hi.yt.spring$closure) + geom_point(aes(y = as.numeric(tot.area),x = year)) + xlab("Year") + ylab(expression(paste("Area (",km^2,")"))) + ylim(c(0,300))
plt.prop.of.hi.scal.area <- ggplot(hi.yt.spring$closure) + geom_point(aes(y = as.numeric(prop.hi.of.scal.area),x = year)) + xlab("Year") + ylab("Proportion") 
...
# So how about a panel showing the Area of 'high' encounter probability on the bank and inside the Canadian Scallop fishery domain.
plt.hi.cod.winter.bank <- ggplot(hi.cod.winter$bank) + geom_point(aes(y = as.numeric(tot.area),x = yrs)) + xlab("Era") + ylab(expression(paste("Area (",km^2,")")))
plt.hi.cod.winter.scal <- ggplot(hi.cod.winter$scal) + geom_point(aes(y = as.numeric(tot.area),x = yrs)) + xlab("Era") + ylab(expression(paste("Area (",km^2,")")))
plt.hi.cod.winter.scal.prop.GB <- ggplot(hi.cod.winter$scal) + geom_point(aes(y = as.numeric(prop.of.GB),x = yrs)) + xlab("Era") + ylab("Porportion")
plt.hi.cod.winter.scal.prop.can <- ggplot(hi.cod.winter$scal) + geom_point(aes(y = as.numeric(prop.scal.hi),x = yrs)) + xlab("Era") + ylab("Porportion")
